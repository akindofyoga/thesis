...
that look identical aside from subtle changes, such as the addition of a single
screw.
...
errors that they were not trained for.
If a user makes a mistake such a mistake, the application will either provide
the user with an instruction for a different step, that it was trained to
recognize, or it might not provide any guidance at all.
Human escalation offers a way to help the user in these cases.

Lastly, reducing the time it takes for developers to build WCA applications for
assembly tasks is imperative for motivating anyone outside of our research group
to build one.
Training computer vision models using synthetic data removes the need to capture
images of each task step and label these images with bounding boxes.

\section{Novelty}

Existing WCA applications used techniques that cannot support realistic assembly
tasks.
...
\item Chapter~\ref{chap:background} discusses prior work on WCA, other work on
  aids for assembly tasks, and the computer vision tasks that we use.
