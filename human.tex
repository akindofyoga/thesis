...
\subsection{Varying the Proportion of Possible Steps}\label{sec:possible}
...
complete.
Users are guaranteed to complete a possible step if they spend enough time on
it.
...
\begin{figure}[h]
  ...
  \caption{
    ...
    error.
    As we describe in Section~\ref{sec:possible}, possible steps are those that
    are completable, but a user may still give up on a possible step and call
    the expert.
...
\begin{figure}[h]
  ...
  \caption{
    The average wait time that resulted from changing the average step length.
    Step length refers to the amount of time it took to complete a step that was
    possible.
    See Section~\ref{sec:possible} for how we define a possible step.
