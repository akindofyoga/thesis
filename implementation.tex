...
voltage values from Android, using the BatteryManager class.
...
Unfortunately, this means that our measurements might not be very accurate.
...
Table TODO lists the power values for each pipeline, running on all three
devices.
...
Table TODO lists the percentage increase in power consumption above the
baseline, for running each pipeline.
...
However, this is still a reasonable amount of power. A 3.2 Wh battery can
supply 1.43 W for over two hours.
