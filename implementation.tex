...
result back for this specific frame will increase.
...
\hl{
  ...
  camera images at 30 FPS should return tokens to clients at the same rate as if
  there were a second cognitive engine that processed frames at 15 FPS.
  ...
  If the frame that was processed is the most recently removed frame from the
  queue, a new frame is removed from the queue and sent to this cognitive engine
  for processing.
  If the processed frame is the most recently removed frame, no new frame is
  removed from the queue, and the cognitive engine is the most recently removed
  frame to process.
...
the baseline application, but less power than running any of the \hl{
  split computing clients.
}
The Google Glass EE 2 consumed a similar amount of power running the thin client
as it did when running the
\hl{
