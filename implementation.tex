\chapter{Device and Cloudlet Implementation}
...
The mobile device immediately starts processing the next image after sending a
cropped image to the cloudlet.
However, we used a single token for Gabriel's flow control, so the mobile client
cannot send a new cropped image to the cloudlet before the cloudlet finishes
processing the previous cropped image.
...
sends the next cropped image to the cloudlet.
...
\end{tabular}
  \caption{
    ...
    These times include the DNN processing times on the device and cloudlet, as
...
Table~\ref{tab:thin_accuracy} lists the accuracies for the thin client, which
runs both DNNs on the cloudlet, and the highest accuracies for mobile-only and
...
latency bound with some of the headsets that we tested.

\subsubsection{Power Consumption}

Power measurements for the thin client, running for five minutes, are listed in
Table~\ref{tab:thin_power}.
...
\end{tabular}
  \caption{
    ...
    For each cell, the average comes before the $\pm$ sign and the standard
    deviation comes after.
    These measurements were recorded while the mobile sent an image to the
    cloudlet, the cloudlet ran the two DNNs, and then sent the result back to
    the mobile device.
    The baseline application did not carry out any computation.
