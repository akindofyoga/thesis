...
For example, a kit might contain two subassemblies that get assembled on their
own, before being connected to the rest of the kit.
...
have been assembled, so there will not be a visible difference between the two
while they are being assembled.
The application can therefore use the same sequence of labels for both
subassemblies.
...
If the next step used the same label, the application would think that the user
