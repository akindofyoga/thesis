...
For each application, we trained a Resnet 50 image classifier, three
different sized EfficientDet object detectors, a Fast MPN-COV classifier,
and a standalone Faster R-CNN object detector.
...
Fast MPN-COV and Faster R-CNN cannot currently run on mobile
...
\hline
  ...
  \hline
  Fast MPN-COV & 73.5\% & 52.0\% & 84\% & \textbf{78.0\%}\\
...
\end{tabular}
  \caption{
    ...
    Resnet 50 and EfficientDet models can be run on a mobile device, while
    Fast MPN-COV and Faster R-CNN can only be run on a cloudlet.
...
improvement.
We next tried to recreate the pipeline described in \S\ref{sec:two_stage}
using an EfficientDet object detector followed by a Resnet 50 classifier.
The Resnet 50 classifiers from \S\ref{sec:single_model} were trained and tested
on uncropped images, while the Resnet 50 classifiers in our pipeline setup were
trained and tested on images that were cropped to just contain the subassembly.
Figure TODO shows an examples of cropped and uncropped images.
We compared these results with a pipeline consisting of Faster R-CNN followed by
Fast MPN-COV.
The results of these pipelines are presented in Table
