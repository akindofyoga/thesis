...
\hl{
  ...
  The first are users, who are completing a task and receiving guidance from the
  application.
  The next are experts, who are familiar with the tasks, and the mistakes that a
  user might make.
  Some systems (including ours) allow experts to help users with a task, over a
  video call.
  The last type of people are developers who create a WCA application for a
  specific task, but developers are not involved at the point that a user
...
\hl{
  created a set of tools that developers can use to
  create WCA applications for assembly tasks.
  ...
  The authors ran a user study to show that their toolchain allows developers to
  create WCA applications faster than the developers could by writing code.
}